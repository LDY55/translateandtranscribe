...
def api_transcribe():
    ...
    try:
        ...
        def transcribe_task(file_list):
            ...
            try:
                ...
                for i, item in enumerate(file_list):
                    ...
                    text_filename = (
                        f"{os.path.splitext(original_name)[0]}_transcript.txt"
                    )
        ...
        return (
            jsonify({"success": False, "error": f"Ошибка транскрибации: {str(e)}"}),
            500,
        )
...
def api_install_pytorch():
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            return (
                jsonify(
                    {"success": False, "error": f"Ошибка установки: {result.stderr}"}
                ),
                500,
            )
        ...
        return (
            jsonify(
                {"success": False, "error": "Тайм-аут установки. Попробуйте позже."}
            ),
            500,
        )
    except Exception as e:
...
def api_download_transcription():
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=transcription_results.txt"
    )
    return response
